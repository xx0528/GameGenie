...
export class RandomMgr {
    ...
    RandomMgr.instance.getRandomInt(min, max, 1) 

    // [min,max] 得到一个两数之间的随机整数，包括两个数在内,这个值比min大（如果min不是整数，那就不小于比min大的整数），但小于（但不等于）max
    RandomMgr.instance.getRandomInt(min, max, 2) 

    // (min,max) 得到一个两数之间的随机整数
    RandomMgr.instance.getRandomInt(min, max, 3) 
     */
     ...
     * @param type  类型
     * @example
    var a = RandomMgr.instance.getRandomByMinMaxList(50, 100, 5)
    console.log("随机的数字", a) 
     */
    ...
    var r = RandomMgr.instance.getRandomByObjectList(b, 5) 
    console.log("原始的对象", b) 
    console.log("随机的对象", r) 
     */
     ...
     * @param sum   随机元素合
     * @example
    var c = RandomMgr.instance.getRandomBySumList(5, -100) 
