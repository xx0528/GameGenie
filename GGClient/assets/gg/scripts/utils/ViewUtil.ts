 ...
 * @LastEditTime: 2023-04-01 19:09:50
 */
...
export class ViewUtil {
    ...
    static createPrefabNode(path: string): Node {
        var p: Prefab = GG.res.get(path, Prefab)! 
        var n = instantiate(p) 
        return n 
    }
    ...
    static createPrefabNodeAsync(path: string): Promise<Node> {
        return new Promise(async (resolve, reject) => {
            GG.res.load(path, Prefab, (err: Error | null, content: Prefab) => {
                if (err) {
    ...
    static addNodeAnimation(path: string, node: Node, onlyOne: boolean = true, isDefaultClip: boolean = false) {
        ...
        var clip = GG.res.get(path, AnimationClip) as AnimationClip 
        if (!clip) {
            return 
        }
