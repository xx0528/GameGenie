 ...
 * @LastEditTime: 2023-03-29 17:33:02
 * @Description: 
 */
...
export class NetNode {
    ...
    protected onConnected(event: any) {
        ...
        GG.log.logNet(`网络已连接当前状态为【${NetNodeStateStrs[this._state]}】`);
    }

    /** 连接验证成功，进入工作状态 */
    protected onChecked() {
        GG.log.logNet("连接验证成功，进入工作状态");
        ...
        if (requests.length > 0) {
            GG.log.logNet(`请求【${this._requests.length}】个待发送的信息`);
    ...
    protected onMessage(msg: any): void {
        // GG.log.logNet(`接受消息状态为【${NetNodeStateStrs[this._state]}】`);
        ...
        if (this._requests.length > 0) {
            ...
            if (this._requests.length == 0) {
                ...
                GG.log.logNet(`请求队列中还有【${this._requests.length}】个请求在等待`);
            }
        }
        ...
        if (null != listeners) {
            for (const rsp of listeners) {
                GG.log.logNet(`触发监听命令【${rspCmd}】的回调`);
    ...
    send(buf: NetData, force: boolean = false): number {
        if (this._state == NetNodeState.Working || force) {
            ...
            GG.log.logNet(`当前状态为【${NetNodeStateStrs[this._state]}】,繁忙并缓冲发送数据`);
            return 0;
        }
        else {
    ...
    private base_request(reqProtocol: IRequestProtocol, rspCmd: string, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
        ...
        GG.log.logNet(`队列命令为【${rspCmd}】的请求，等待请求数据的回调`);
