 ...
 * @LastEditTime: 2023-04-01 19:22:04
 */
...
export class EffectSingleCase {
    ...
    constructor() {
        GG.eventMgr.on(EffectEvent.Put, this.onHandler, this)
    }
    ...
    loadAndShow(name: string, parent?: Node, params?: IEffectParams): Promise<Node> {
        return new Promise(async (resolve, reject) => {
            ...
            if (np == undefined) {
                GG.res.load(name, Prefab, (err: Error | null, prefab: Prefab) => {
                    if (err) {
    ...
    /**
     * 清除对象池数据
     * @param name  参数为空时，清除所有对象池数据指定名时，清楚指定数据
     */
    clear(name?: string) {
        if (name) {
            var np = this.effects.get(name)!
